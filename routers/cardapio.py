...
def obter_estabelecimento_por_slug(cursor, slug: str):
    ...
    return est['id'] if isinstance(est, tuple) else est['id']
...
@router.post("/{slug}/api/cardapio/adicionar")
async def api_adicionar_produto(
    slug: str,
    nome: str = Form(...),
    descricao: str = Form(""),
    preco: float = Form(...),
    ...
    try:
        ...
        return JSONResponse({"status": "sucesso", "mensagem": "Produto adicionado com sucesso!"})
    except Exception as e:
        return JSONResponse({"status": "erro", "detalhe": str(e)}, status_code=500)

@router.post("/{slug}/api/cardapio/arquivar/{id}")
def api_arquivar_produto(slug: str, id: int):
    try:
        db = get_db()
        cursor = db.cursor()
        est_id = obter_estabelecimento_por_slug(cursor, slug)
        ...
        cursor.close()
        db.close()
        return JSONResponse({"status": "sucesso", "mensagem": "Produto arquivado!"})
    except Exception as e:
        return JSONResponse({"status": "erro", "detalhe": str(e)}, status_code=500)

@router.post("/{slug}/api/cardapio/desarquivar/{id}")
def api_desarquivar_produto(slug: str, id: int):
    try:
        db = get_db()
        cursor = db.cursor()
        est_id = obter_estabelecimento_por_slug(cursor, slug)
        ...
        cursor.close()
        db.close()
        return JSONResponse({"status": "sucesso", "mensagem": "Produto desarquivado!"})
    except Exception as e:
        return JSONResponse({"status": "erro", "detalhe": str(e)}, status_code=500)

@router.post("/{slug}/api/cardapio/toggle_visibilidade/{id}")
def api_toggle_visibilidade(slug: str, id: int):
    try:
        db = get_db()
        cursor = db.cursor()
        est_id = obter_estabelecimento_por_slug(cursor, slug)
        ...
        cursor.close()
        db.close()
        return JSONResponse({"status": "sucesso", "mensagem": "Visibilidade alterada!"})
    except Exception as e:
        return JSONResponse({"status": "erro", "detalhe": str(e)}, status_code=500)

@router.post("/{slug}/api/cardapio/excluir/{id}")
def api_excluir_produto(slug: str, id: int):
    try:
        db = get_db()
        cursor = db.cursor()
        est_id = obter_estabelecimento_por_slug(cursor, slug)
        ...
        cursor.close()
        db.close()
        return JSONResponse({"status": "sucesso", "mensagem": "Produto excluído com sucesso!"})
    except Exception as e:
        return JSONResponse({"status": "erro", "detalhe": str(e)}, status_code=500)
